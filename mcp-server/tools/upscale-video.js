...
import {
    ...
    getAccountInstanceId, fetchRecaptchaToken, reloadRecaptchaPage,
...
export async function handler(params, extra) {
    ...
    // the page is reloaded — manually reproducing this confirmed even the official UI
    // fails on the same page until F5. The outer retry below does that reload programmatically.
    ...
    for (let outerAttempt = 0; outerAttempt <= MAX_OUTER_RETRIES; outerAttempt++) {
        ...
        const upResult = await submitVideoUpscale(token, projectId, mediaId, aspectRatio, recaptchaToken, instanceId, sessionCookies);
        if (!upResult.upsampledMediaId) {
            lastError = new Error('Upscale submit returned no upsampledMediaId');
            continue;
        }
        ...
        try {
            ...
            upsampledMediaId = upResult.upsampledMediaId;
            break;
        } catch (e) {
            lastError = e;
            ...
            console.error(`[FlowVideo] Poll failed (${e.message}) — will reload page and retry once`);
        }
    }
    ...
    if (!upsampledMediaId) {
        ...
        return { content: [{ type: 'text', text: `⚠️ Video upscale failed: ${lastError?.message || 'unknown error'}` }] };
    }

    await progress('Getting download URL...');
    const videoUrl = await fetchVideoDownloadUrl(token, projectId, upsampledMediaId, sessionCookies, instanceId);
