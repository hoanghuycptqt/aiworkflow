...
- **Slow-path reload success = `/session` validates ALIVE, NOT "the session-token rotated"** (`a585221`, 2026-06-04 incident, memory `vps-flow-slowpath-validate-alive`): under FEX the Flow SPA loads in 80–100s, so the FIRST session-token rotation (~12s) is usually the dead **error-embedding JWE** (`ACCESS_TOKEN_REFRESH_NEEDED`) — the SUCCESSFUL re-grant lands ~8s later (~20s). `profile_reload.py` now keeps Firefox alive and polls `/session` (`_session_alive`, mirrors `services/flow-session.js getAccessToken`) until a token validates alive; returning on the first rotation handed the connector a dead token → false "needs re-login" every ~20h. The Mac never hit this — `mcp-server/lib/firefox-refresh.js` waits a fixed 20s + Rosetta loads faster.
...
**Slow-path semantics — "rotated" ≠ "alive", and a still-dead refresh ≠ always re-login** (2026-06-04, memory `vps-flow-slowpath-validate-alive`): the broker reload's success signal is a `/session`-validated **alive** token, NOT a changed session-token — NextAuth re-encrypts the JWE even when it only embeds a refresh *error*, so a rotation is not proof of life (this caused a 20h-cycle false "needs re-login"; fixed in `a585221`, see the connector invariant above). And a slow refresh that STILL returns `ACCESS_TOKEN_REFRESH_NEEDED` does **not** automatically mean a revoked grant: if the profile's **Google account cookies** (`SID`/`SAPISID`/`__Secure-1PSID`/`__Secure-3PSID`/`LSID`, ~13-month life) are still valid, a patient standalone-Firefox page-load **silently re-auths server-side in ~20s** — no `accounts.google.com` redirect, no click, no 2FA. The genuine ~2-month Telegram number-tap 2SV re-login is required ONLY when those Google account cookies themselves expire. **Diagnostic recipe** when "🔑 Token hết hạn" recurs: (1) the three profile paths (Telegram login, slow reload, connector) ALL resolve to `BROKER_PROFILE_BASE/<accountId>` with `accountId = email.replace(/[^a-zA-Z0-9]/g,'_').toLowerCase()` — a path mismatch is NOT the cause (ruled out 2026-06-04); (2) ground truth = assemble the profile cookies (`profile_cookies.read_profile_cookies(profile_dir)`) and `curl https://labs.google/fx/api/auth/session` — an `error` field + a past `expires` = dead token, and `user.email` confirms the account; (3) check `SID`/`SAPISID` expiry in `cookies.sqlite` — if still valid the session is **recoverable** (slow-path/timing issue), not a dead grant. To watch a reload live: noVNC is bound to `127.0.0.1:6080` (not public) → `ssh -i <repo>/cert/ssh-key-2026-06-01.key -L 6080:127.0.0.1:6080 truonghoanghuy@149.118.142.16 -N`, then `http://localhost:6080/vnc.html` (display `:99`).
